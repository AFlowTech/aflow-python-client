...
class AHandleParam(AFlowBaseModel):
...
class AComboxDataGroup(AFlowBaseModel):
...
class AFormData(AFlowBaseModel):
    values: List[AFieldValue] = Field(..., description="全表单字段值")


class HandleFlowReq(AFlowBaseModel):
    """
    处理标准表单
    """
...
class PropertyMappingConfig(AFlowBaseModel):
...
class HandleFlowByObjectReq(AFlowBaseModel):
    """
    处理泛型表单
    """
...
if hasattr(AComboxDataGroup, "model_rebuild"):
    AComboxDataGroup.model_rebuild()
else:
